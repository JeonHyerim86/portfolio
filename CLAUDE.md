...
| 무엇을 | 어디서 | 규칙 |
|--------|--------|------|
| **콘텐츠** (문구·프로젝트·성과 수치·연락처·이력) | **[docs/prd.md](docs/prd.md)** | 사실을 임의로 지어내지 않는다. 반드시 PRD 근거. |
| **디자인·애니메이션·홈페이지 구조(틀)** | **[docs/design-ex.md](docs/design-ex.md)** | 비주얼 언어, 모션, 섹션 구성은 이 스펙을 따른다. |
...
- **테두리 굴곡(입체감) 복원 후처리**: 위 디컨탬(안쪽 색을 바깥으로 블리드)은 헤일로를 없앴지만 외곽 몇 px이 **평평한 단색 띠**가 되어 3D 렌더 특유의 가장자리 말림(form falloff)이 사라졌다. 이를 복원하기 위해 `ref/3d-profile.png`(검정 배경 원본, 992×1075 — 같은 렌더의 저해상 버전)를 1280×1387로 업스케일해 정렬(오프셋 dx=12, dy=0, 내부 평균 색차 3.7/255로 동일 렌더 검증)한 뒤, **실루엣 경계 5px 밴드 안에서만** 거리 기반 가중치(경계 100% → 5px 안쪽 0%)로 원본 색을 블렌드해 웨이브 가닥의 가장자리 음영을 되살렸다. **알파(실루엣) 무변경, 밴드 밖 내부 픽셀 무변경**(변경 40,572px 전부 경계 4.5px 이내 검증). 밝은 배경에서도 halo 없음 확인.
