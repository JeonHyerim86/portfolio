...
// 내용은 전혜림의 실제 스택(Spring Boot·JPA·MySQL·Docker·성능 최적화)이라
...
public class OrderService {
    ...
    public OrderResponse place(OrderRequest req) {
        ...
        PaymentResult result = paymentClient.charge(order.getId(), order.total(), req.method());
        eventPublisher.publishEvent(new OrderPlacedEvent(order.getId(), result.transactionId()));
        log.info("order placed id={} amount={} tx={} elapsedMs={}", order.getId(), order.total(), result.transactionId(), order.elapsedMillis());
        return OrderResponse.from(order, result);
    }
}`,
  `public interface OrderRepository extends JpaRepository<Order, Long> {
    @Query("select o from Order o join fetch o.items where o.status = :status and o.createdAt >= :from order by o.createdAt desc")
    List<Order> findRecentByStatus(@Param("status") OrderStatus status, @Param("from") LocalDateTime from, Pageable pageable);
}`,
  `-- 성능 최적화: 복합 인덱스 추가로 상태별 최근 주문 조회 TPS 약 50% 개선
CREATE INDEX idx_orders_status_created ON orders (status, created_at DESC, user_id);
EXPLAIN ANALYZE SELECT o.id, o.status, o.total_amount, u.name FROM orders o JOIN users u ON u.id = o.user_id WHERE o.status = 'PAID' ORDER BY o.created_at DESC LIMIT 50;`,
  `@GetMapping("/api/orders")
public Page<OrderSummary> list(@RequestParam OrderStatus status, @PageableDefault(size = 20, sort = "createdAt") Pageable pageable) {
    return orderService.search(status, pageable).map(OrderSummary::from);
}`,
  `spring:
  datasource:
    url: jdbc:mysql://localhost:3306/shop?rewriteBatchedStatements=true&useServerPrepStmts=true
    hikari: { maximum-pool-size: 20, connection-timeout: 3000, minimum-idle: 5 }
  jpa:
    hibernate: { ddl-auto: validate }
    properties: { hibernate.jdbc.batch_size: 50, hibernate.order_inserts: true }`,
